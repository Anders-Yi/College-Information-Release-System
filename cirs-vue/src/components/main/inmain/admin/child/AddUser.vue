<template>
  <div id="adduser">
    <div class="adduser-card">
      <div class="adduser-title">添加用户</div>
      <span class="close" @click="closeAddUser">X</span>
      <form action="#" method="post" class="form-table">
        ...
        <input type="button" @click="submitAddUser" class="adduser-btn" value="添加用户">
      </form>
    </div>
  </div>
</template>
    ...
    submitAddUser: function() {
      ...
      if(flag && this.departId == '0'){
        ...
        if(this.uid.startsWith(this.clazzId)){
          console.log("发送用户注册请求",this.uid.startsWith(this.clazzId));
          this.$axios({
